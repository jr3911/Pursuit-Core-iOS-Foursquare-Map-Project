...
class VenueFetchingService {
    ...
    func getVenues(lat: Double, long: Double, query: String, completionHandler: @escaping (Result<[Venue], AppError>) -> () ){
        ...
        let urlString = "https://api.foursquare.com/v2/venues/explore?client_id=\(Secrets().clientId)&client_secret=\(Secrets().clientSecret)&v=20191106&limit=2&ll=\(lat),\(long)&query=\(query)"
